...
// this is a modified version that only extracts text from a specific target area by
// comparing the Word quads to the user specified area.  See //DLADD comments
//
...
//Step 3) Extract unicode from a second PDF document into a text file 
    ...
    if (outputFile.is_open())
    {
		...
		//DLADD
		ASFixedRect sourcePageSize;
		ASFixedRect targetRegion;
		// user specified region to extract text in points (origin of the page is bottom left)
		float userTargetRegionL = 0;    // left
		float userTargetRegionB = 0;    // bottom
		float userTargetRegionR = 200;  // right
		float userTargetRegionT = 792;  // top   (0,0,200,792) is a rectangle encompassing the  left side of the page
		// convert them to ASFixed
		...
		PDPageGetCropBox(pdPage, &sourcePageSize);

		// The origin of a PDF page is not always 0, 0. If the user target values were specified from a
		// visual representation of the page, rather than from internal coordinate values, you might need
		// to adjust the user specified values by the amount of the page origin (left and bottom).
		userTargetRegionFixedL = userTargetRegionFixedL + sourcePageSize.left;
		userTargetRegionFixedB = userTargetRegionFixedB + sourcePageSize.bottom;
		userTargetRegionFixedR = userTargetRegionFixedR + sourcePageSize.left;
		userTargetRegionFixedT = userTargetRegionFixedT + sourcePageSize.bottom;
        ...
        for (ASInt32 index = 0; index < numWords; ++index)
        {
			...
			//DLADD
			ASFixedQuad fullWordQuad;
			ASFixedQuad wordQuad;
			
			PDWord pdWord = PDWordFinderGetNthWord(pdWordFinder, index); \
			// printf("\nNumquads = %d\n", PDWordGetNumQuads(pdWord));
			PDWordGetNthQuad(pdWord, 0, &fullWordQuad); // add the first quad to the full word quad.

			ASFixed centerH;
			ASFixed centerV;
			// There are different options for determining whether a Word should be captured:
			//   * if the complete Word fits within the target rect
			//   * if any part of the Word fits within the target
			//   * if the midpoit of the Word fits within the rect
			// For this test, we will consider a word to be in the target rectangle if the midpoint 
			// of any of it's quads intersects the target area
			...
			for (int i = 0; i < PDWordGetNumQuads(pdWord); i++)
			{
				...
				centerH = (wordQuad.bl.h + wordQuad.br.h + wordQuad.tr.h + wordQuad.tl.h) / 4;
				centerV = (wordQuad.bl.v + wordQuad.br.v + wordQuad.tr.v + wordQuad.tl.v) / 4;
				// printf("Word # %d coords: %f  %f", index, ASFixedToFloat(centerH), ASFixedToFloat(centerV));
